...
class modeloModel extends criaPaginacao{
	...
	public function setTipoDados($x){
		$codeUtf8 = htmlentities(utf8_decode($x));//servidot online
	...
	public function setCondWhere($x){$y = self::setRemoveArray($x);
		...
		if($contArray == 1){
			...
			if($urlEncont[2] == "page"){
				$cond = "url_categoria='$categoria' && url_subcategoria='$subCat'";
			}else{
